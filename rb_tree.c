 ...
 *
 */
static void rb_tree_left_rotate(rb_tree_t *tree, rb_tree_node_t *x)
{
    rb_tree_node_t *y;
    y = x->right;
    x->right = y->left;
 ...
 * 
 */
static void rb_tree_right_rotate(rb_tree_t *tree, rb_tree_node_t *y)
{
    rb_tree_node_t *x;
    x = y->left;
    y->left = x->right;
...
static int rb_tree_delete_fixup_left(rb_tree_t *tree, rb_tree_node_t **node, rb_tree_node_t **parent)
{
    ...
    if ((!temp->left || temp->left->color == BLACK) &&
        ...
        if (!temp->right || temp->right->color == BLACK) {
            ...
            temp->color = RED;
            rb_tree_right_rotate(tree, temp);
            temp = (*parent)->right;
        }
...
static void rb_tree_node_free(rb_tree_node_t *node, void(*kfree)(void *), void (*vfree)(void *))
{
    if (kfree && node->key) kfree(node->key);
    if (vfree && node->value) vfree(node->value);
    free(node);
}
...
{
    rb_tree_node_t *node = rb_tree_search(tree, key);
...
void rb_tree_foreach(rb_tree_t *tree, void(void *, void *, void *), void *arg)
{
